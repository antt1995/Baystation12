...
// above is for borgs, this is for the mappable structure!
#define MAX_ROLLER_BEDS 4
/obj/structure/roller_rack
	...
	var/list/obj/item/roller_bed/held = list()
	var/initial_beds = 0
...
/obj/structure/roller_rack/Initialize()
	...
	for (var/i in 1 to initial_beds)
		var/obj/item/roller_bed/bed = new(src)
		held += bed
	update_icon()
...
/obj/structure/roller_rack/on_update_icon()
	...
	var/image/base_rack = overlay_image(icon, "rack_base")
	if (material.icon_colour)
		base_rack.color = material.icon_colour
	base_rack.alpha = 255 * material.opacity
	AddOverlays(base_rack)

	for (var/beds in 1 to length(held))
		var/image/I = overlay_image(icon, "bed_[beds]")
		if (beds != MAX_ROLLER_BEDS)
			var/image/rack = overlay_image(icon, "rack_[beds]")
			if (material.icon_colour)
	...
	var/image/final_rack = overlay_image(icon, "rack_4")
	if (material.icon_colour)
		final_rack.color = material.icon_colour
	final_rack.alpha = 255 * material.opacity
	AddOverlays(final_rack)
	desc = "[initial(desc)]\nIt is holding [length(held)] beds."
...
/obj/structure/roller_rack/attack_hand(mob/user)
	...
	var/obj/item/roller_bed/bed = held[length(held)]
...
/obj/structure/roller_rack/use_tool(obj/item/tool, mob/user, list/click_params)
	...
	if (istype(tool, /obj/item/roller_bed))
		var/obj/item/roller_bed/bed = tool
		if (length(held) >= MAX_ROLLER_BEDS)
			to_chat(user, SPAN_NOTICE("The rack has no space for \the [bed]."))
			return
		user.drop_from_inventory(bed, src)
		held += bed
		to_chat(user, SPAN_NOTICE("You place \the [bed] on the rack."))
		update_icon()
#undef MAX_ROLLER_BEDS
